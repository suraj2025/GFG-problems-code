...
class Solution {
    static Node deleteAllOccurOfX(Node head, int x) {
        ...
        // Remove all occurrences of `x` at the beginning of the list
        while (head != null && head.data == x) {
            head = head.next; // Move the head pointer forward
            if (head != null) head.prev = null; // Update prev pointer
        }

        // Handle the case where the list becomes empty after removal
        if (head == null) return null;
